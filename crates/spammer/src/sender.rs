...
use crate::result_tracker::TxOutcome;
...
pub(crate) struct TxSender {
    ...
    tx_source: TxSource,
    result_sender: Sender<TxOutcome>,
...
impl TxSender {
    ...
    async fn run_channel(&mut self) -> Result<()> {
        ...
        loop {
            ...
            for err in pending_errors {
                // Connection-level / transport errors aren't tied to a specific
                // tx, so pass 0 bytes; only counts as a sent submission.
                let _ = self.result_sender.send((0, Err(err))).await;
            }
    ...
    async fn send(&mut self, tx: TxEnvelope, wait_response: bool) -> Result<ChannelSendOutcome> {
        ...
        if wait_response {
            ...
            self.result_sender
                .send((tx_len, response.map(|_: String| ())))
                .await?;
            Ok(outcome)
        } else {
            ...
            self.result_sender.send((tx_len, Ok(()))).await?;
    ...
    async fn send_request_with_retry(
        ...
        result_sender: &mut Sender<TxOutcome>,
        reconnect_attempts: u32,
        reconnect_period: Duration,
    ) -> Result<u64> {
            ...
            match ws_client.request(method, params.clone()).await {
                ...
                Err(e) if is_connection_error(&e) => {
                    ...
                    if let Err(reconnect_err) = ws_client.reconnect().await {
                        ...
                        // Connection retry failure isn't tied to a single tx, so
                        // pass 0 bytes; only the offered-count is incremented.
                        if attempt + 1 >= max_attempts {
                            ...
                            result_sender.send((0, Err(err))).await?;
        ...
        let err = eyre::eyre!("Failed to send request after {} attempts", max_attempts);
        result_sender.send((0, Err(err))).await?;
