...
/// Key for pending proposal parts.
pub type PendingPartsKey = (HeightKey, RoundKey, BlockHashKey);

/// Key for undecided blocks.
pub type UndecidedBlockKey = (HeightKey, RoundKey, BlockHashKey);

/// Key representing a height.
...
impl redb::Value for BlockHashKey {
    ...
    type AsBytes<'a> = [u8; BlockHash::len_bytes()];

    fn fixed_width() -> Option<usize> {
        Some(BlockHash::len_bytes())
    }
    ...
    where
        Self: 'a,
    {
        let bytes = <Self::AsBytes<'a> as redb::Value>::from_bytes(data);
        BlockHash::new(bytes)
    }
...
impl redb::Key for BlockHashKey {
    fn compare(data1: &[u8], data2: &[u8]) -> std::cmp::Ordering {
        <Self::AsBytes<'_> as redb::Key>::compare(data1, data2)
    }
}
