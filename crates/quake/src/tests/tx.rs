...
fn transfer_test<'a>(
    ...
    Box::pin(async move {
        ...
        // Build, sign, and send a self-transfer from the test mnemonic account
        let raw_tx = sign_self_transfer(&target_client, account_index).await?;
...
/// Builds and signs an EIP-1559 self-transfer from the test mnemonic account
/// at `account_index`, querying its current (pending) nonce from `client`.
/// Returns the EIP-2718-encoded raw transaction, ready for `send_raw_transaction`.
pub(super) async fn sign_self_transfer(
