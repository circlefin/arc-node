...
impl Terraform {
    ...
    fn build_variables(
        ...
        // Global images only, used for a cloud-init warmup `docker pull`.
        // Mixed-version networks still pull each node's real image at `docker
        // compose up` via the per-node compose file.
        args.push("-var".to_string());
