...
use arc_consensus_db::RangeFailureReason;
...
pub(crate) struct RouteDef {
    ...
    /// Admin routes. Mounted only when the admin RPC is enabled.
    pub admin: bool,
}
...
macro_rules! route {
    // Read-only routes must be GET. A non-GET route without the `admin` marker
    // is rejected by the catch-all arm below, so a state-mutating route cannot
    // be registered as publicly reachable by forgetting to tag it.
    (get, $path:expr, $handler_fn:path, $desc:expr) => {
        route!(@build get, $path, $handler_fn, $desc, None, false)
    };
    (get, $path:expr, $handler_fn:path, $desc:expr, params = { $( $pkey:expr => $pval:expr ),* $(,)? }) => {
        route!(@build get, $path, $handler_fn, $desc, Some(::std::collections::BTreeMap::from([ $( ($pkey, $pval) ),* ])), false)
    };
    // Admin routes use any method but must opt in explicitly via `admin`.
    (admin $method:ident, $path:expr, $handler_fn:path, $desc:expr) => {
        route!(@build $method, $path, $handler_fn, $desc, None, true)
    };
    ...
    // Any non-admin route that is not GET is almost certainly an oversight: it
    // would expose a non-read-only endpoint without the admin gate. Reject it.
    ($method:ident, $($rest:tt)*) => {
        compile_error!(concat!(
            "non-admin RPC route `",
            stringify!($method),
            "` must be GET; mark state-mutating routes with the `admin` keyword (e.g. `admin ",
            stringify!($method),
            "`)"
        ))
    };
...
/// Query parameters shared by the height-range observability endpoints
/// (`/commit`, `/misbehavior-evidence`, `/proposal-monitor`,
/// `/invalid-payloads`): an optional anchor `height` and an optional `count`
/// of consecutive heights to return.
#[derive(serde::Deserialize)]
pub(crate) struct HeightRangeParams {
...
/// Why a range query could not be served, as rendered on the wire.
///
/// `OverLimit` is RPC-layer policy (the request never reaches the store); the
/// other four map one-to-one from the store's [`RangeFailureReason`].
#[derive(Serialize, Copy, Clone)]
#[serde(rename_all = "snake_case")]
pub(crate) enum RpcRangeReason {
    AboveCurrentHead,
    Pruned,
    NotRecorded,
    Internal,
...
impl From<RangeFailureReason> for RpcRangeReason {
    fn from(reason: RangeFailureReason) -> Self {
        match reason {
            RangeFailureReason::AboveCurrentHead => Self::AboveCurrentHead,
            RangeFailureReason::Pruned => Self::Pruned,
            RangeFailureReason::NotRecorded => Self::NotRecorded,
            RangeFailureReason::Internal => Self::Internal,
        }
    }
}

/// The inclusive height range the client asked for (`from..=to`), echoed back
/// in the error so a failed poll can be retried without re-deriving the bounds.
#[derive(Serialize)]
...
/// Structured `400` body for a range query that cannot be fully served.
///
/// Uniform across all four endpoints so the polling client handles failures
/// identically and retries only `failed_heights`.
#[derive(Serialize)]
pub(crate) struct RangeError {
    error: &'static str,
    requested: RequestedRange,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    failed_heights: Vec<u64>,
    reason: RpcRangeReason,
}

/// Build the structured `400` for an unavailable or over-limit range.
///
/// `failed_heights` is empty for `over_limit` (the range is never evaluated).
pub(crate) fn range_error(
    from: u64,
    to: u64,
    reason: RpcRangeReason,
    failed_heights: Vec<Height>,
) -> (StatusCode, Json<RangeError>) {
    (
        StatusCode::BAD_REQUEST,
        Json(RangeError {
            error: "partial range unavailable",
            requested: RequestedRange { from, to },
            failed_heights: failed_heights.iter().map(Height::as_u64).collect(),
            reason,
        }),
    )
}
