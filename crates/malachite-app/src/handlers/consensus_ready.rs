...
use crate::payload::{
    check_payload_binding, EnginePayloadValidator, PayloadValidationResult, PayloadValidator,
};
...
async fn replay_block(
    ...
    // The handshake takes the consensus position from the block number of the
    // replayed payload, so a payload that does not chain to `previous_block`
    // moves that position. Stop instead.
    if let Err(error) = check_payload_binding(&payload, height, Some(previous_block)) {
        metrics.inc_binding_halt_count(BindingHaltSite::Replay);

        return Err(eyre!(
            "Replay: payload {payload_hash} is not bound to its place in the chain at height {height}: {error}"
        )
        .into());
    }
...
async fn handshake_and_replay(
    ...
    while replay_height <= latest_height_cons {
        let previous_block = latest_block;
...
mod tests {
    ...
    /// A stored payload that reports another block number moves the consensus
    /// position the handshake derives from it, so the handshake stops instead.
    #[tokio::test]
    async fn replay_stops_on_a_payload_that_reports_another_height() {
        let el_height = 4u64;
        let cl_height = Height::new(el_height + 1);
        let latest_block_el = test_execution_block(el_height, 1000);

        // The payload for height 5 reports block number 2.
        let payload_to_replay = test_payload(2, 1100);
        ...
        let error = handshake_and_replay(
            &payload_validator,
            &block_finalizer,
            &engine_api,
            &ethereum_api,
        ...
        .await
        .expect_err("an unbound stored payload must stop the handshake");

        assert!(
            error
                .to_string()
                .contains("is not bound to its place in the chain"),
            "got: {error:#}",
        );
        assert_eq!(metrics.get_binding_halt_count(BindingHaltSite::Replay), 1);
    }
