...
use crate::opcode::{
    arc_network_selfdestruct, arc_network_selfdestruct_zero7, arc_network_selfdestruct_zero8,
};
...
fn insert_arc_selfdestruct_instruction<DB: Database>(
    instruction: &mut EthInstructions<EthInterpreter, EthEvmContext<DB>>,
    hardfork_flags: ArcHardforkFlags,
) {
    if hardfork_flags.is_active(ArcHardfork::Zero8) {
        instruction.insert_instruction(
            SELFDESTRUCT,
            Instruction::new(arc_network_selfdestruct_zero8, 5000),
        );
...
impl<CTX: ContextTr, INSP, I, P> ArcEvm<CTX, INSP, I, P> {
    ...
    fn check_blocklist_and_create_log(
        ...
        // Probe inside a checkpoint once the cold-state preserving behavior is active, so this
        // unmetered selfdestructed-target check does not leave a previously-cold account warm.
    ...
    PRECOMPILE: PrecompileProvider<EthEvmContext<DB>, Output = InterpreterResult>,
{
    ...
    fn transact_system_call(
        ...
        // System calls skip handler pre_execution, so warm NativeCoinControl here.
        // Running through ArcEvmHandler preserves system-call fee semantics while applying
        // Arc frame/opcode hooks, including fail-closed blocklist reads during SELFDESTRUCT.
        self.inner
            .ctx
...
impl EvmFactory for ArcEvmFactory {
    ...
    fn create_evm<DB: Database>(&self, db: DB, mut input: EvmEnv) -> Self::Evm<DB, NoOpInspector> {
        ...
        insert_arc_selfdestruct_instruction(&mut instruction, hardfork_flags);
...
mod tests {
    ...
    use arc_execution_config::chainspec::{localdev_with_hardforks, DEVNET, LOCAL_DEV, TESTNET};
    use arc_precompiles::precompile_provider::ArcPrecompileProvider;
    ...
    fn pre_zero7_chain_spec() -> Arc<ArcChainSpec> {
    ...
    fn mark_blocklisted(db: &mut InMemoryDB, address: Address) {
        let slot = native_coin_control::compute_is_blocklisted_storage_slot(address);
    ...
    fn selfdestruct_to_bytecode(target: Address) -> Bytecode {
    ...
    #[test]
    fn test_system_call_preserves_basic_system_call_semantics() {
        let chain_spec = LOCAL_DEV.clone();
    ...
    fn test_zero7_system_call_uses_arc_frame_init_for_nested_blocklist_checks() {
        ...
        mark_blocklisted(&mut db, blocklisted_target);
        let mut evm = create_arc_evm(chain_spec, db);
    ...
    fn test_system_call_preloads_native_coin_control_for_selfdestruct_blocklist() {
        ...
        let system_contract = Address::repeat_byte(0x21);
        let blocklisted_target = Address::repeat_byte(0x22);
        let runtime = selfdestruct_to_bytecode(blocklisted_target);
    ...
    fn test_pre_zero7_system_call_preserves_nested_call_blocklist_bypass() {
        let chain_spec = pre_zero7_chain_spec();
        let system_contract = Address::repeat_byte(0x21);
        let blocklisted_target = Address::repeat_byte(0x22);
        let transfer_amount = U256::from(7);
        let runtime = call_with_value_bytecode(blocklisted_target, transfer_amount);
        ...
        assert!(
            result.result.is_success(),
            "pre-Zero7 system-call nested CALL should bypass Arc frame blocklist hooks"
        );
        let target_balance = result
            .state
            .get(&blocklisted_target)
    ...
    fn test_pre_zero7_system_call_preserves_selfdestruct_blocklist_fail_open() {
        ...
        assert!(
            result.result.is_success(),
            "pre-Zero7 system call should preserve old fail-open SELFDESTRUCT behavior"
        );
    }
    ...
    fn create_to_cold_target_preserves_cold_status(
        ...
        assert_eq!(
            is_account_cold(&evm, target_address),
            expected_target_is_cold,
            "unexpected create target warmth after selfdestruct probe for hardforks {hardforks:?}"
        );
    }
