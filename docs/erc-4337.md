# ERC-4337 Account Abstraction on Arc Testnet
  ...
  Arc Testnet is EVM-compatible but has three constraints that affect ERC-4337 contract deployment and operation. All three have silent failure modes — no helpful revert message, just a failed transaction or rejected UserOperation.

  | Constraint | Detail | Why it matters for ERC-4337 |
  |---|---|---|
  ...
  Arc Testnet rejects the `PUSH0` opcode (`0x5f`), which `solc ≥ 0.8.20` emits by default when targeting the Shanghai EVM or later.
  ...
  **solc directly / programmatic compilation** (e.g., via `solc-js`):
  ...
  Symptom when missing: the deployment transaction succeeds on-chain but the deployed bytecode is zero bytes (or the transaction reverts silently). Verify with:

  ```bash
  cast code <deployed_address> --rpc-url https://rpc.testnet.arc.io
  ```

  #### 2. Keep `entryPoint` as a `constant`, not `immutable`

  Solidity encodes each `immutable` variable in the constructor's init code. A contract with **two or more** `immutable` variables emits a `0x60c0` init-code prefix. Arc Testnet (and some other chains) silently revert this init code, deploying zero bytecode.

  The ERC-4337 reference Paymaster stores `entryPoint` as `immutable`. If you add a second `immutable` (e.g., a USDC address), the contract will fail to deploy on Arc.

  **Fix:** Declare `entryPoint` (or whichever variable you can predetermine) as a Solidity `constant`:

  ```solidity
  // ❌ Fails on Arc — two immutables → 0x60c0 init → silent revert
  IEntryPoint private immutable entryPoint;
  IERC20       private immutable usdc;

  // ✅ Works on Arc — one immutable → 0x60a0 init
  address public constant ENTRY_POINT =
      0x0000000071727De22E5E9d8BAf0edAc6f37da032;
  IERC20 private immutable usdc;       // only one immutable → safe
  ```

  Symptom when missing: deployment transaction is mined but `cast code` returns `0x`.

  #### 3. Do not use `nonReentrant` on `validatePaymasterUserOp`
  ...
  |---|---|---|
  | Deployment succeeds but `cast code` returns `0x` | `PUSH0` in bytecode (Shanghai+ EVM target) | Compile with `evmVersion: "paris"` |
  | Deployment succeeds but `cast code` returns `0x` | Two `immutable` variables (0x60c0 init) | Change one to `constant` |
