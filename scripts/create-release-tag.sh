...
usage() {
  cat <<'USAGE'
Usage:
  create-release-tag.sh --source <ref> --release-kind patch|minor|major [--as-release-candidate] [--tag-prefix v] [--release-branch-prefix release/] [--main-branch main]
  create-release-tag.sh --tag <existing tag> [--tag-prefix v] [--release-branch-prefix release/] [--main-branch main]
...
normalize_branch_name() {
  local ref="$1"
  ref="${ref#origin/}"
  if [[ "${ref}" == "${MAIN_BRANCH}" || "${ref}" == "${RELEASE_BRANCH_PREFIX}"* ]]; then
    echo "${ref}"
    return
  fi
  ...
  containing="$(
    while IFS= read -r branch; do
      branch="${branch#origin/}"
      if [[ "${branch}" == "${MAIN_BRANCH}" || "${branch}" == "${RELEASE_BRANCH_PREFIX}"* ]]; then
        echo "${branch}"
        break
      fi
...
latest_final_tag_for_candidate() {
  ...
  local candidate_version major minor tag version tag_filter
...
next_tag() {
  local kind="$1"
  local source="$2"
  local latest latest_version major minor patch rc latest_is_rc tag_filter branch_name base_tag
  ...
  if [[ "${AS_RELEASE_CANDIDATE}" == true ]]; then
    if [[ "${latest_is_rc}" == true ]] && continues_rc_line "${kind}" "${minor}" "${patch}"; then
  ...
  if [[ "${kind}" == "patch" && "${latest}" != "${base_tag}" && "${latest_is_rc}" == false ]] && tag_points_to_ref "${latest}" "${source}"; then
    echo "Reusing ${latest}; source commit is already tagged" >&2
    printf '%s\n' "${latest}"
    return
  fi
...
validate_release_source() {
  ...
  if [[ "${source_ref}" == "${RELEASE_BRANCH_PREFIX}"* ]]; then
