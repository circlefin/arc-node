...
normalize_branch_name() {
  local ref="$1"
  ref="${ref#origin/}"
  if [[ "${ref}" == "${BASE_BRANCH}" || "${ref:0:${#RELEASE_BRANCH_PREFIX}}" == "${RELEASE_BRANCH_PREFIX}" ]]; then
    echo "${ref}"
    return
  fi
  ...
  containing="$(
    while IFS= read -r branch; do
      branch="${branch#origin/}"
      if [[ "${branch}" == "${BASE_BRANCH}" || "${branch:0:${#RELEASE_BRANCH_PREFIX}}" == "${RELEASE_BRANCH_PREFIX}" ]]; then
        echo "${branch}"
        break
      fi
...
ref_is_contained_by_release_branch() {
  local ref="$1"
  local branch
  ...
  done < <(git branch -r --contains "${ref}" --format='%(refname:short)' 2>/dev/null)

  while IFS= read -r branch; do
    if release_line_from_branch "${branch}"; then
      return 0
    fi
  done < <(git branch --contains "${ref}" --format='%(refname:short)' 2>/dev/null)
...
validate_release_source() {
  ...
  local source_arg_branch="${source_arg#origin/}"

  if [[ "${kind}" == "patch" ]]; then
    if [[ "${source_arg_branch}" == "${BASE_BRANCH}" ]]; then
      echo "patch releases must start from ${RELEASE_BRANCH_PREFIX}X.Y or an explicit commit contained by a ${RELEASE_BRANCH_PREFIX}X.Y branch, not ${source_arg}" >&2
      exit 1
    fi

    if release_line_from_branch "${source_branch}" ||
      release_line_from_branch "${source_arg_branch}" ||
      ref_is_contained_by_release_branch "${resolved_source_ref}"; then
      return
    fi
  ...
  if release_line_from_branch "${source_branch}"; then
    echo "${kind} releases must start from ${BASE_BRANCH} or an explicit ${BASE_BRANCH} commit, not ${source_branch}" >&2
    exit 1
  fi

  if [[ "${source_arg_branch:0:${#RELEASE_BRANCH_PREFIX}}" == "${RELEASE_BRANCH_PREFIX}" ]]; then
    echo "${kind} releases must not use release branch source_ref=${source_arg}" >&2
    exit 1
  fi
}
...
if [[ -n "${RELEASE_KIND}" ]]; then
  ...
  validate_release_source "${RELEASE_KIND}" "${SOURCE_REF}" "${RESOLVED_SOURCE_REF}" "${SOURCE_BRANCH}"
...
CHANNEL="preview"
if git merge-base --is-ancestor "${SHA}" "origin/${BASE_BRANCH}" 2>/dev/null || git merge-base --is-ancestor "${SHA}" "${BASE_BRANCH}" 2>/dev/null; then
  CHANNEL="latest"
