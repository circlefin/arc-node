...
- **[EL] JSON-RPC insufficient-balance error messages changed with the reth 2.2 / revm 38 upgrade.** `eth_call` and `eth_estimateGas` on transactions whose value or gas cost exceeds the sender balance now surface revm 38's `OutOfFunds` text (was `insufficient funds for gas * price + value`); simple transfers surface `gas required exceeds allowance` (was `Missing or invalid parameters`). See [BREAKING_CHANGES.md](./BREAKING_CHANGES.md#unreleased).
